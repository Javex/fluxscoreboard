...
% if items:
    ...
    % for news in items:
            <tr>
                ...
                <td>${news.message if len(news.message) <= 50 else news.message[:50] + "..."}</td>
                <td>
                % if news.challenge:
                ...
                <td class="btn-group">
                    ...
                    <ul class="dropdown-menu">
                        <li><a href="${request.route_url('admin_news_edit', id=news.id)}">Edit</a></li>
                        <li><a href="${request.route_url('admin_news_delete', id=news.id)}">Delete</a></li>
                        <li>
                            <a href="${request.route_url('admin_news_toggle_status', id=news.id)}">
                                % if news.published:
...
${admin_funcs.display_admin_form('admin_news', form, "Announcement", is_new)}
